...
function PricingCard({ bono, index }: { bono: typeof bonos[0]; index: number }) {
  ...
  return (
    <motion.div
      ...
      className={`relative group ${bono.popular ? "z-10 lg:scale-105" : ""}`}
    >
      {/* Animated border glow for popular */}
      {bono.popular && (
        <motion.div
          className="absolute -inset-[2px] rounded-[26px] bg-gradient-to-r from-blue-600 via-cyan-400 to-blue-600 opacity-70 blur-sm -z-10"
          animate={{
      ...
      {/* Card Container */}
      <motion.div
        animate={isHovered ? { y: -10, scale: 1.02 } : { y: 0, scale: 1 }}
        transition={{ type: "spring", stiffness: 300, damping: 20 }}
        className={`relative overflow-hidden rounded-3xl ${
          bono.popular
            ? "glass-extreme border-2 border-blue-400/50 shadow-xl shadow-blue-500/20"
            : "glass-extreme border border-white/40"
        }`}
      >
        ...
        <motion.div
          ...
          className={`absolute inset-0 bg-gradient-to-br ${bono.color} opacity-5`}
          style={{ backgroundSize: "200% 200%" }}
        />
        ...
        {bono.popular && (
          <>
            <motion.div
              ...
              className="absolute top-10 right-10 w-20 h-20 rounded-full bg-gradient-to-br from-blue-500/20 to-cyan-400/20 blur-xl"
            />
            <motion.div
              animate={{ y: [10, -10, 10], x: [5, -5, 5] }}
              transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
              className="absolute bottom-20 left-5 w-16 h-16 rounded-full bg-gradient-to-br from-cyan-400/20 to-blue-500/20 blur-xl"
            />
          </>
        )}
          ...
          <div className="text-center mb-8">
            ...
            >
              ...
              <div className="flex items-center justify-center gap-2 text-slate-500">
                ...
                <span className="text-sm">Validez 1 ano</span>
              </div>
            </motion.div>
          </div>
          ...
          <div className="text-center mb-8">
            ...
            <div className="mt-2">
              <span className="inline-block glass px-3 py-1 rounded-full text-sm font-medium text-blue-600">
                {bono.pricePerSession} euros/sesion
              </span>
            </div>
          </div>
    ...
    <section id="bonos" className="py-24 relative overflow-hidden" aria-labelledby="bonos-title">
      ...
      <div className="absolute inset-0">
        <motion.div
          ...
          className="absolute top-1/4 -left-20 w-[600px] h-[600px] rounded-full bg-gradient-to-br from-blue-500/10 to-cyan-400/5 blur-3xl"
        />
        <motion.div
          animate={{ 
            scale: [1.2, 1, 1.2],
          ...
          className="absolute bottom-1/4 -right-20 w-[500px] h-[500px] rounded-full bg-gradient-to-br from-cyan-400/10 to-blue-500/5 blur-3xl"
        />
      </div>
          ...
          <h2 id="bonos-title" className="text-3xl sm:text-4xl lg:text-5xl font-bold text-slate-900 mb-6 text-balance">
            ...
            <span className="relative">
              ...
              <motion.span
                ...
                className="absolute -bottom-2 left-0 right-0 h-1 bg-gradient-to-r from-blue-600 to-cyan-500 rounded-full origin-left"
              />
            </span>
          </h2>
